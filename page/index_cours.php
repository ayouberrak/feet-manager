...
// ATTENTION: La requête SQL utilise $filtre sans requête préparée, 
// créant une vulnérabilité d'Injection SQL dans le filtre.
...
if ($filtre == "all") {
    ...
    $filtre_safe = mysqli_real_escape_string($conn, $filtre);
    $sql = "SELECT * FROM COURS WHERE categorie = '$filtre_safe'";
}
...
<body>
    ...
    <div class="sidebar">
        ...
        <nav>
            ...
            <a href="index_cours.php" class="active">📚 Gérer les Cours</a>
    ...
    <div class="main-container">
        ...
        <div class="main-content">
            <div class="content-card">
                ...
                <a href="./add/add_cours.php" class="add-button">➕ Ajouter un cours</a>

                <form method="GET" action="" class="filter-form">
                    <label for="filtre">Filtrer par catégorie :</label>
                    <select name="filtre" id="filtre">
                        <option value="all" <?= $filtre == 'all' ? 'selected' : '' ?>>Toutes les Catégories</option>
                        <option value="Musculationn" <?= $filtre == 'Musculationn' ? 'selected' : '' ?>>Musculation</option>
                        <option value="Yoga" <?= $filtre == 'Yoga' ? 'selected' : '' ?>>Yoga</option>
                    </select>
                    ...
                    <tbody>
                        <?php while ($row = mysqli_fetch_assoc($result)): ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars($row['date_cours']) ?></td>
                                <td><?= htmlspecialchars($row['heure_cours']) ?></td>
                                <td><?= htmlspecialchars($row['duree']) ?></td>
                                <td><?= htmlspecialchars($row['max_participants']) ?></td>

                                <td>
                                    <a href="./update/update_cours.php?id=<?= $row['id_cours'] ?>" class="action-link edit">✏️ Edit</a> 
                                    <span style="color:#ccc; margin-right:5px;">|</span>
                                    <a href="./delete/delete_cours.php?id=<?= $row['id_cours'] ?>" class="action-link delete" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce cours ?')">🗑️ Delete</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
